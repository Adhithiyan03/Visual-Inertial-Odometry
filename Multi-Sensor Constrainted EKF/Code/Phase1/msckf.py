...
class MSCKF(object):
    ...
    def feature_callback(self, feature_msg):
        ...
        try:
            ...
            if not os.path.exists('stamped_traj_estimate.txt'):
                with open('stamped_traj_estimate.txt', 'w') as f:
                    f.write('# timestamp tx ty tz qx qy qz qw\n')
            
            with open('stamped_traj_estimate.txt', 'a') as f:
                f.write(f'{feature_msg.timestamp} ')
                f.write(f'{self.state_server.imu_state.position[0]} ')
                f.write(f'{self.state_server.imu_state.position[1]} ')
                f.write(f'{self.state_server.imu_state.position[2]} ')
                f.write(f'{self.state_server.imu_state.orientation[0]} ')
                f.write(f'{self.state_server.imu_state.orientation[1]} ')
                f.write(f'{self.state_server.imu_state.orientation[2]} ')
                f.write(f'{self.state_server.imu_state.orientation[3]}\n')
    ...
    def initialize_gravity_and_bias(self):
        ...
        for i in range(len(self.imu_msg_buffer)):
            sum_linear_acc += self.imu_msg_buffer[i].linear_acceleration
            sum_angular_vel += self.imu_msg_buffer[i].angular_velocity
        ...
        R = Rotation.align_vectors(gravity_imu.reshape((1,3)), -IMUState.gravity.reshape((1,3)))[0]
        # R = R.T
        # Initialize the initial orientation, so that the estimation
        # is consistent with the inertial frame.
        self.state_server.imu_state.orientation = Rotation.from_matrix(R.as_matrix().T).as_quat()
    ...
    def batch_imu_processing(self, time_bound):
        ...
        used_imu_msg_cntr = 0
        for i in range(len(self.imu_msg_buffer)):
            imu_time = self.imu_msg_buffer[i].timestamp
            ...
            self.process_model(imu_time, self.imu_msg_buffer[i].angular_velocity, self.imu_msg_buffer[i].linear_acceleration)
            used_imu_msg_cntr += 1
    ...
    def predict_new_state(self, dt, gyro, acc):
        ...
        if gyro_norm > 1e-5:
            dq_dt = np.cos(gyro_norm*dt/2) * np.identity(4) + np.sin(gyro_norm*dt/2)/gyro_norm * Omega
            dq_dt = dq_dt @ q
            dq_dt2 = np.cos(gyro_norm*dt/4) * np.identity(4) + np.sin(gyro_norm*dt/4)/gyro_norm * Omega 
            dq_dt2 = dq_dt2 @ q
        else:
            dq_dt = np.cos(gyro_norm*dt/2) * np.identity(4) + 0.5*dt * Omega 
            dq_dt = dq_dt @ q
            dq_dt2 = np.cos(gyro_norm*dt/4) * np.identity(4) + 0.25*dt * Omega
            dq_dt2 = dq_dt2 @ q
    ...
    def state_augmentation(self, time):
        ...
        state_cov[old_rows:, old_rows:] = J @ state_cov[:21, :21] @ J.T
        # Fix the covariance to be symmetric
    ...
    def add_feature_observations(self, feature_msg):
        ...
        for i in range(len(feature_msg.features)):
            feature = feature_msg.features[i]
    ...
    def measurement_update(self, H, r):
        ...
        if not H.shape[0] or not r.shape[0]:
            return
        ...
        if (np.linalg.norm(delta_x_imu[6:9]) > 0.5 or 
            ...
            print('Update change is too large: ', np.linalg.norm(delta_x_imu))
        ...
        I_KH = np.identity(K.shape[0]) - K @ H_thin
